function capacity = decode_with_capture_bsma(nodes,tdiag_ap,packet_duration, gateway_position, CE_Thresh)
...
for j = 2:size(tdiag_ap,1)-1
    if(tdiag_ap(j,1)-tdiag_ap(j-1,1)==1)    % A packet has arrived at AP
        if(tdiag_ap(j+1,2)-tdiag_ap(j,2)==packet_duration)
            ...
            activeGw1hot = gateway_position.nmap(:,coll_nodes);
            activeGwIdx = sum(activeGw1hot,2);
            activeGwIdx = find(activeGwIdx).';
            ...
            for cellIdx = 1:num_cells
                ...
                for btoneGwIdx = activeGwIdx
                    if btoneGwIdx == cellIdx
                        continue
                    else
                        RSS = [RSS getRSS(gateway_position.x(btoneGwIdx),gateway_position.y(btoneGwIdx),gateway_position.x(cellIdx),gateway_position.y(cellIdx))];
                    end
                end

                % Inter-gateway interference
